...
def init_settings(settings_json_path: Path) -> None:
    ...
    process_name = Path(settings.get("game_info", {}).get("game_exe_path", "")).name
...
def get_game_exe_path() -> Path | None:
    game_exe_path = Path(
        settings_information.settings.get("game_info", {}).get(
    "game_exe_path", None,
))
...
def get_game_launcher_exe_path() -> Path | None:
...
def get_uproject_file() -> Path | None:
    ...
    if not raw_path or not settings_dir.is_dir():
        return None

    if not raw_path.is_absolute():
        return (settings_dir/ raw_path).resolve()
    else:
        return raw_path.resolve()
...
def get_engine_packaging_args() -> list:
    ...
    if unreal_engine.is_game_ue4(get_unreal_engine_dir()):
...
def get_default_release_dir() -> Path:
    dir_to_return =  Path(f'{Path.cwd()}/Modding/releases')
    dir_to_return.mkdir(parents=True, exist_ok=True)
    return dir_to_return


def get_default_release_base_files_dir() -> Path:
